...
function applyRandomSoftTheme() {
  const seed =
    softThemeSeeds[Math.floor(Math.random() * softThemeSeeds.length)];
...
function checkAllLoaded() {
  attemptedImages++;
  if (attemptedImages === leafImageUrls.length) {
    if (leafImages.length === 0) {
      leafImages.push(createFallbackLeaf());
    }
    isLoading = false;
    initLeaves();
...
leafImageUrls.forEach((url) => {
  ...
  img.onload = () => {
    ...
    try {
      ...
      // Fallback for environments where canvas pixel reads are restricted.
      leafImages.push(img);
    }
    checkAllLoaded();
  };
...
function createFallbackLeaf() {
  ...
  offCtx.beginPath();
  offCtx.ellipse(16, 16, 10, 6, Math.PI / 4, 0, Math.PI * 2);
  offCtx.fill();
  return offCanvas;
}
...
function createTintedLeaf(originalImage) {
  ...
  // Tint using compositing so we never need getImageData() (blocked on file://).
  offCtx.globalCompositeOperation = "source-atop";
  offCtx.globalAlpha = 0.8;
  offCtx.fillStyle = config.leafColor;
...
class Leaf {
  ...
  update(deltaTime) {
    ...
    this.angle =
      this.baseAngle + Math.sin(this.swayOffset) * this.rotationRange;
    if (this.y > document.documentElement.scrollHeight) {
      ...
      this.swayOffset = random(0, Math.PI * 2);
      this.swaySpeed = random(
        config.swayFrequency.min,
        config.swayFrequency.max,
      );
    }
...
function animate(currentTime) {
  ...
  const deltaTime = Math.min(currentTime - lastTime || 16.67, 32);
...
function typeLoop() {
  ...
  if (isDeleting) {
    ...
    if (charIndex === 0) {
      ...
      setTimeout(typeLoop, pauseBefore);
      return;
    }
